...
var dorita980 = function localV2 (user, password, host, emitIntervalTime, connectTimeout) {
  ...
  connectTimeout = connectTimeout || 30000; // 30 second default timeout
  var robotState = {};
  var cap = null;
  var missionInterval;
  ...
  // Support multiple cipher suites for different Roomba models (j7+, j9+, etc.)
  // AES128-SHA256 works for most, but newer models may need alternatives
  const ciphers = process.env.ROBOT_CIPHERS || 'AES128-SHA256:AES128-SHA:AES256-SHA256:AES256-SHA';
  ...
  var options = {
    ...
    reconnectPeriod: 0 // Disable auto-reconnect, let consumer control this
  };

  const client = mqtt.connect(url, options);

  // Error events are now properly emitted instead of thrown
  // Consumers should listen: robot.on('error', (e) => {...})
  // If no error listener is attached, Node.js will emit 'uncaughtException'
